...
`output/<プロジェクト名>/<YYYYMMDD>/`配下に書き出す。ディレクトリの
階層を深くしすぎると結果を辿るのが手間になるため、実行時刻は
ディレクトリではなく**ファイル名に付与する**（例:
`customer_pref_summary_143205.html`、
`customer_pref_summary_profile_143205.json`）。これにより、同日に
複数回実行しても上書きされず、かつディレクトリは日単位までに留められる。

`output/<プロジェクト名>/index.html`は、そのプロジェクトの生成物の目次
（実行日時・ツール名と、各`<YYYYMMDD>/`配下のファイルへのリンクの
一覧）で、`file://`で開いてそのままブラウザ遷移で辿れるようにする。
実行のたびに新しいエントリを追記して更新する。
