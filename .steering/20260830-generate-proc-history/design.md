...
- **`eqp_per_ope_name`を`{6,10}`→`{8,15}`に、`lot_count`を`5000`→`6000`に
  変更。あわせて「できれば設備を全部使いたい」という要望に応えるため、
  `_build_ope_eqp_candidates()`のアルゴリズムを変更した**：
  従来は`ope_no`ごとに全`eqp_id`から独立に`k`台を抽選しており、
  `eqp_count`を増やしても一部の設備が一度も選ばれない可能性があった
  （実測: 400台中341台のみ使用）。そこで「①全`eqp_id`をシャッフルして
  `ope_name_pool`にラウンドロビンで1台ずつ配る（全`eqp_id`が必ずどこかの
  `ope_no`の候補に最低1回入ることを構造的に保証）→②`ope_no`ごとの目標
  台数に届いていない分を、まだ入っていない`eqp_id`から追加抽選する」
  という2段階方式に変更した。ラウンドロビンで既に目標台数を超えている
  場合は間引かない（カバレッジの保証を優先し、候補数が目標よりやや
  多くなることを許容する）。`eqp_count=400`・`lot_count=6000`で
  再生成し、400/400台全てが実際に使われたことをDuckDBで確認済み。
