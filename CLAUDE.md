...
- 実処理は `src/analyse_tool/<name>/` にツールごとのサブパッケージとして置き、
  ...
  1. **前準備 (`prepare.py`)** — 読み込んだ生データの整形（型変換・欠損値
     処理・不要行の除外など）。分析に入る前の下ごしらえ。
  2. **データ加工 (`process.py`)** — 結合・集約・特徴量作成など、
     分析対象のデータそのものを作る変換処理。
  3. **分析 (`analyze.py`)** — 加工済みデータに対する集計・統計量算出・
     比較など、数値としての分析結果を作る処理。
  4. **可視化 (`visualize.py`)** — 分析結果をグラフ・表など見せる形に
     変換・出力する処理。
...
1. `src/analyse_tool/<ツール名>/` にサブパッケージを作り、`cli.py` /
   `io.py` に加えて `prepare.py`（前準備）/ `process.py`（データ加工）/
   `analyze.py`（分析）/ `visualize.py`（可視化）の4ステップに分けて
   実装する。入出力パスや挙動は `argparse` などでオプション化し、
   決め打ちにしない。
